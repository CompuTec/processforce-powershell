...
$csvTestsProtocolsPropertiesItemCertifiacteOfAnalysisPath = -join ($csvImportCatalog, "Quality_TestProtocolsPropertiesItemCertifiacteOfAnalysis.csv");
$csvTestsProtocolsPropertiesTestPath = -join ($csvImportCatalog, "Quality_TestProtocolsPropertiesTest.csv");
$csvTestsProtocolsPropertiesTestCertifiacteOfAnalysisPath = -join ($csvImportCatalog, "Quality_TestProtocolsPropertiesTestCertifiacteOfAnalysis.csv");
...
try {
	...
	[array] $csvTestsProtocolsPropertiesItemCerts = $null
	if ((Test-Path -Path $csvTestsProtocolsPropertiesItemCertifiacteOfAnalysisPath -PathType leaf) -eq $true) {
		[array] $csvTestsProtocolsPropertiesItemCerts = Import-Csv -Delimiter ';' -Path $csvTestsProtocolsPropertiesItemCertifiacteOfAnalysisPath
	}
	else {
	...
	[array] $csvTestsProtocolsPropertiesTestCerts = $null
	if ((Test-Path -Path $csvTestsProtocolsPropertiesTestCertifiacteOfAnalysisPath -PathType leaf) -eq $true) {
		[array] $csvTestsProtocolsPropertiesTestCerts = Import-Csv -Delimiter ';' -Path $csvTestsProtocolsPropertiesTestCertifiacteOfAnalysisPath
	}
	else {
		write-host "Properties Certificate Of Analysis - csv not available."
	}
	...
	$totalRows = $csvTestsProtocols.Count + $csvTestsProtocolsItems.Count + $csvTestsProtocolsPropertiesItem.Count + $csvTestsProtocolsPropertiesTest.Count + $csvTestsProtocolsResources.Count + $csvTestsProtocolsAttachments.Count + $csvTestsProtocolsPropertiesItemCerts.Count + $csvTestsProtocolsPropertiesTestCerts.Count;
	...
	$dictionaryCertsItem = New-Object 'System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[array]]';
	$dictionaryPropertiesTest = New-Object 'System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[array]]';
	$dictionaryCertsTest = New-Object 'System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[array]]';
	...
	foreach ($row in $csvTestsProtocolsPropertiesItemCerts) {
		$key = $row.TestProtocolCode;
		$progressItterator++;
		$progres = [math]::Round(($progressItterator * 100) / $total);
		if ($progres -gt $beforeProgress) {
		...
		if ($dictionaryCertsItem.ContainsKey($key) -eq $false) {
			$dictionaryCertsItem[$key] = New-Object System.Collections.Generic.List[array];
		}
		$list = $dictionaryCertsItem[$key];
	...
	foreach ($row in $csvTestsProtocolsPropertiesTestCerts) {
		$key = $row.TestProtocolCode;
		$progressItterator++;
		$progres = [math]::Round(($progressItterator * 100) / $total);
		if ($progres -gt $beforeProgress) {
		...
		if ($dictionaryCertsTest.ContainsKey($key) -eq $false) {
			$dictionaryCertsTest[$key] = New-Object System.Collections.Generic.List[array];
		}
		$list = $dictionaryCertsTest[$key];
