...
$csvSubstitutesLinesPath = -join ($csvImportCatalog, "SubstitutesLines.csv")
...
try {   
    ...
    [array] $csvSubstitutesLines = Import-Csv -Delimiter ';' $csvSubstitutesLinesPath;
	
    write-Host 'Preparing data: '
    $totalRows = $csvSubstitutes.Count + $csvSubstitutesLines.Count;
    $substitutesList = New-Object 'System.Collections.Generic.List[array]'
    $substitutesLinesDict = New-Object 'System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[array]]'
    ...
    foreach ($row in $csvSubstitutesLines) {
        $key = $row.Code;
        $progressItterator++;
        $progres = [math]::Round(($progressItterator * 100) / $total);
        if ($progres -gt $beforeProgress) {
        ...
        if ($substitutesLinesDict.ContainsKey($key) -eq $false) {
            $substitutesLinesDict[$key] = New-Object System.Collections.Generic.List[array];
        }
        $list = $substitutesLinesDict[$key];
    ...
    foreach ($csvHeader in $substitutesList) {
        try {
            ...
            [array]$csvItems = $substitutesLinesDict[$csvHeader.Code]
