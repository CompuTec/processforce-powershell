...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Complaint Reasons
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Complaint Reasons. Script add new or will update existing Reasons.
...
$mdsCsvPath = -join ($csvImportCatalog, "ComplaintReasons.csv")
...
try {
    ...
    $mds = Import-Csv -Delimiter ';' -Path $mdsCsvPath;
    ...
    foreach ($md in $mds) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Complaint with Code {1} Details: {2}", $taskMsg, $md.ComplaintResonCode, $err);
