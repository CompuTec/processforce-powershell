...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Ingredient Categories
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Ingredient Categories. Script add new or will update existing Categories.
...
$csvIngredientCategoriesPath = -join ($csvImportCatalog, "IngredientCategories.csv")
...
try {
    ...
    $csvHeaders = Import-Csv -Delimiter ';' -Path $csvIngredientCategoriesPath;
    ...
    foreach ($csvHeader in $csvHeaders) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Ingredient Category with Code {1} Details: {2}", $taskMsg, $csvHeader.Code, $err);
