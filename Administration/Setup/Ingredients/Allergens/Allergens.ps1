...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Alergens
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Allergens. Script add new or will update existing Allergens.
...
$csvAllergensPath = -join ($csvImportCatalog, "Allergens.csv")
...
try {
    ...
    $csvHeaders = Import-Csv -Delimiter ';' -Path $csvAllergensPath;
    ...
    foreach ($csvHeader in $csvHeaders) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OALG"" WHERE ""U_Code"" = N'{0}'", $csvHeader.Code));
	
            #Creating object
            $md = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::Allergen)
