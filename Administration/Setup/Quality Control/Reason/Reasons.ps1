...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Reasons
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Reasons. Script add new or will update existing Reasons.
...
$csvReasonsPath = -join ($csvImportCatalog, "Reasons.csv")
...
try {
    ...
    $mds = Import-Csv -Delimiter ';' -Path $csvReasonsPath;
    ...
    foreach ($md in $mds) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_ORSN"" WHERE ""U_RsnCode"" = N'{0}'", $md.ReasonCode));
	
            #Creating object
            $mdObj = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::Reason)
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Reason with Code {1} Details: {2}", $taskMsg, $md.ReasonCode, $err);
