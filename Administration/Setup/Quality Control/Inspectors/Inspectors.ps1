...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Inspectors
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Inspectors. Script add new or will update existing Inspectors.
...
$csvInspectorsPath = -join ($csvImportCatalog, "Inspectors.csv")
...
try {
    ...
    $mds = Import-Csv -Delimiter ';' -Path $csvInspectorsPath;
    ...
    foreach ($md in $mds) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OINS"" WHERE ""U_InsCode"" = N'{0}'", $md.InspectorCode));
	
            #Creating object
            $mdObj = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::Inspector)
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Inspector with Code {1} Details: {2}", $taskMsg, $md.InspectorCode, $err);
