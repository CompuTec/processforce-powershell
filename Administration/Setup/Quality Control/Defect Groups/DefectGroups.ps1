...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Defects Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Defects Groups. Script add new or will update existing Defects Groups.
...
$csvDefectGroupsPath = -join ($csvImportCatalog, "DefectGroups.csv")
...
try {
    ...
    $Grps = Import-Csv -Delimiter ';' -Path $csvDefectGroupsPath;
    ...
    foreach ($grp in $Grps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_ODEG"" WHERE ""U_DefGrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::DefectGroup)
            ...
            if ($rs.RecordCount -gt 0) {
                ...
                $group.U_DefGrpCode = $grp.GroupCode;
                $exists = $false
            }
   
            $group.U_DefGrpName = $grp.GroupName;
            $group.U_DefGrpRemarks = $grp.Remarks;
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Defect Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
