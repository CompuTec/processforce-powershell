...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Inspector Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Inspector Groups. Script add new or will update existing Inspector Groups.
...
$csvInspectorGroupsPath = -join ($csvImportCatalog, "InspectorGroups.csv")
...
try {
    ...
    $Grps = Import-Csv -Delimiter ';' -Path $csvInspectorGroupsPath;
    ...
    foreach ($grp in $Grps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OING"" WHERE ""U_InsGrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::InspectorGroup)
            ...
            if ($rs.RecordCount -gt 0) {
                ...
                $group.U_InsGrpCode = $grp.GroupCode;
                $exists = $false
            }
   
            $group.U_InsGrpName = $grp.GroupName;
            $group.U_InsGrpRemarks = $grp.Remarks;
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Inspector Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
