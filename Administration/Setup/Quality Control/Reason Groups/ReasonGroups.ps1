...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Reasons Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Reasons Groups. Script add new or will update existing Groups.
...
$csvReasonGroupsPath = -join ($csvImportCatalog, "ReasonGroups.csv")
...
try {
    ...
    $Grps = Import-Csv -Delimiter ';' -Path $csvReasonGroupsPath;
    ...
    foreach ($grp in $Grps) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Reason Groups with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
