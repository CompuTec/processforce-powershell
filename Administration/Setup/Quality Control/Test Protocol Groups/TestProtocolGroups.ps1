...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Test Protocol Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Test Protocol Groups. Script add new or will update existing Groups.
...
$csvTestProtocolGroupsPath = -join ($csvImportCatalog, "TestProtocolGroups.csv")
...
try {
    ...
    $Grps = Import-Csv -Delimiter ';' -Path $csvTestProtocolGroupsPath;
    ...
    foreach ($grp in $Grps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OTCG"" WHERE ""U_GrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::TestProtocolGroup)
            ...
            $group.U_GrpRemarks = $grp.Remarks;
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Test Protocol Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
