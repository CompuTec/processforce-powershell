...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import NCMR Reasons
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import NCMR Reasons. Script add new or will update existing NCMR Reasons.
...
$csvReasonsNCMRPath = -join ($csvImportCatalog, "ReasonsNCMR.csv")
...
try {
    ...
    $mds = Import-Csv -Delimiter ';' -Path $csvReasonsNCMRPath;
    ...
    foreach ($md in $mds) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} NCMR Reason with Code {1} Details: {2}", $taskMsg, $md.ReasonCode, $err);
