...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Test Property Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Test Property Groups. Script add new or will update existing Groups.
...
$csvTestPropertyGroupsPath = -join ($csvImportCatalog, "TestPropertyGroups.csv")
...
try {
    ...
    $Grps = Import-Csv -Delimiter ';' -Path $csvTestPropertyGroupsPath;
    ...
    foreach ($grp in $Grps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OTPG"" WHERE ""U_TestPrpGrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::TestPropertyGroup)
            ...
            if ($rs.RecordCount -gt 0) {
                ...
                $group.U_TestPrpGrpCode = $grp.GroupCode;
                $exists = $false
            }
   
            $group.U_TestPrpGrpName = $grp.GroupName;
            $group.U_TestPrpGrpRemarks = $grp.Remarks;
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Test Property Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
