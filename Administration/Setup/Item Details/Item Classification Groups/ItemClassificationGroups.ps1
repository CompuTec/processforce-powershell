...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Item Classificaton Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Item Classifications Groups. Script add new or will update existing Groups.
...
$csvItemClassificationGroupsPath = -join ($csvImportCatalog, "ItemClassificationGroups.csv")
...
try {
    ...
    $ICGrps = Import-Csv -Delimiter ';' -Path $csvItemClassificationGroupsPath;
    $rs = $pfcCompany.CreateSapObject([SAPbobsCOM.BoObjectTypes]"BoRecordset")

    if ($ICGrps.Count -gt 1) {
        $total = $ICGrps.Count
    }
    else {
        $total = 1
    ...
    foreach ($grp in $ICGrps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OICG"" WHERE ""U_GrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Item Classification Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::ItemClassificationGroup)
            ...
            if ($rs.RecordCount -gt 0) {
                ...
                $group.U_GrpCode = $grp.GroupCode;
                $exists = $false
            }
   
            $group.U_GrpName = $grp.GroupName;
            $group.U_GrpDescription = $grp.Remarks;
	
            $message = 0
            #Adding or updating Item Classification Groups depends on exists in the database
            if ($exists -eq $true) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Item Classification Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
