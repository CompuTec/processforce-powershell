...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Item Phrase Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Item Phrases Groups. Script add new or will update existing Groups.
...
$csvItemPhraseGroupsPath = -join ($csvImportCatalog, "ItemPhraseGroups.csv")
...
try {
    ...
    $ICGrps = Import-Csv -Delimiter ';' -Path $csvItemPhraseGroupsPath;
    ...
    foreach ($grp in $ICGrps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OIHG"" WHERE ""U_GrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Item Classification Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::ItemPhrasesGroup)
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Item Phrase Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
