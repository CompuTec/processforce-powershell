...
$csvPropertyReferencesPath = -join ($csvImportCatalog, "PropertyReferences.csv")
...
try {
    ...
    [array]$itmPropRefs = Import-Csv -Delimiter ';' -Path $csvPropertyReferencesPath;
    $rs = $pfcCompany.CreateSapObject([SAPbobsCOM.BoObjectTypes]"BoRecordset")
    if ($itmPropRefs.Count -gt 1) {
        $total = $itmPropRefs.Count
    }
    else {
        $total = 1
    }
    ...
    foreach ($ref in $itmPropRefs) {
        try {
            ...
            if ($rs.RecordCount -gt 0) {
                $dummy = $propertyReference.GetByKey($rs.Fields.Item(0).Value);
                $exists = $true;
            }
            else {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Property Reference with Code {1} Details: {2}", $taskMsg, $ref.ReferenceCode, $err);
