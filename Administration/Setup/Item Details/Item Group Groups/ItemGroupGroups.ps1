...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Group Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 10 RL:07 ('') '930.240.18.14' (64-bit)
# Description:
#      Import Group Groups. Script add new or will update existing Groups.
...
$csvItemGroupGroupsPath = -join ($csvImportCatalog, "ItemGroupGroups.csv")
...
try {
    ...
    $ITGrps = Import-Csv -Delimiter ';' -Path $csvItemGroupGroupsPath;
    $rs = $pfcCompany.CreateSapObject([SAPbobsCOM.BoObjectTypes]"BoRecordset")
    if ($ITGrps.Count -gt 1) {
        $total = $ITGrps.Count
    }
    else {
        $total = 1
    ...
    foreach ($grp in $ITGrps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OIGG"" WHERE ""U_GrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Item Group Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::ItemGroupGroup)
            ...
            $message = 0
            #Adding or updating Item Group Groups depends on exists in the database
            if ($exists -eq $true) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Item Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
