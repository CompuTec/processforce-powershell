...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Text Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Text Groups. Script add new or will update existing Groups.
...
$csvItemTextGroupsPath = -join ($csvImportCatalog, "ItemTextGroups.csv")
...
try {
    ...
    $ITGrps = Import-Csv -Delimiter ';' -Path $csvItemTextGroupsPath;
    ...
    foreach ($grp in $ITGrps) {
        try {
            ...
            $rs.DoQuery([string]::Format("SELECT ""Code"" FROM ""@CT_PF_OITG"" WHERE ""U_GrpCode"" = N'{0}'", $grp.GroupCode));
	
            #Creating Item Text Group object
            $group = $pfcCompany.CreatePFObject([CompuTec.ProcessForce.API.Core.ObjectTypes]::ItemTextGroup)
            ...
            $message = 0
            #Adding or updating Item Text Groups depends on exists in the database
            if ($exists -eq $true) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Item Test Group with Code {1} Details: {2}", $taskMsg, $grp.GroupCode, $err);
