...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Resource Properties
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Resource Properties. Script add new or will update existing Resource Properties.
...
$csvResourcePropertiesPath = -join ($csvImportCatalog, "ResourceProperties.csv")
...
try {
    ...
    $resProps = Import-Csv -Delimiter ';' -Path $csvResourcePropertiesPath
    if ($resProps.Count -gt 1) {
        $total = $resProps.Count
    }
    else {
        $total = 1
    }
    ...
    foreach ($prop in $resProps) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Resource Properties with Code {1} Details: {2}", $taskMsg, $prop.PropertyCode, $err);
