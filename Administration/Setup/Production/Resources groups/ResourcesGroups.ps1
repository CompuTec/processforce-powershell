...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Resource Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Resource Groups. Script add new or will update existing RefereGroupsnces.
...
$csvResourcesGroupsPath = -join ($csvImportCatalog, "ResourcesGroups.csv")
...
try {
    ...
    $csvItems = Import-Csv -Delimiter ';' -Path $csvResourcesGroupsPath
    ...
    foreach ($csvItem in $csvItems) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Resource Group with Code {1} Details: {2}", $taskMsg, $csvItem.ResourceGrpCode, $err);
