...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Downtime Reasons Groups
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Downtime Reasons Groups. Script add new or will update existing Groups.
...
$csvDowntimeReasonGroupsPath = -join ($csvImportCatalog, "DowntimeReasonGroups.csv")
...
try {
    ...
    [array] $csvDowntimeReasonGroups = Import-Csv -Delimiter ';' -Path $csvDowntimeReasonGroupsPath
    ...
    foreach ($item in $sourceList) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Downtime Reasons Group with Code {1} Details: {2}", $taskMsg, $item.Code, $err);
