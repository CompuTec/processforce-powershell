...
Clear-Host
########################################################################
# CompuTec PowerShell Script - Import Operation Properties
########################################################################
$SCRIPT_VERSION = "3.0"
# Last tested PF version: ProcessForce 9.3 (9.30.140) PL: 04 R1 HF1 (64-bit)
# Description:
#      Import Operation Properties. Script add new or will update existing Properties.
...
$csvOperationPropertiesPath = -join ($csvImportCatalog, "OperationProperties.csv")
...
try {
    ...
    $oprProps = Import-Csv -Delimiter ';' -Path $csvOperationPropertiesPath

    if ($oprProps.Count -gt 1) {
        $total = $oprProps.Count
    }
    else {
        $total = 1
    }
    ...
    foreach ($prop in $oprProps) {
        ...
        Catch {
            ...
            $ms = [string]::Format("Error when {0} Operation Property with Code {1} Details: {2}", $taskMsg, $prop.PropertyCode, $err);
